...
# in DrawMode, users can use their mouse to doodle
class DrawMode(Mode):
    def appStarted(mode):
    ...
    def redrawAll(mode, canvas):
        if (mode.waitingForFirstKeyPress):
            ...
            canvas.create_text(mode.width/2, mode.height*7/18, 
                            ...
                            font = 'Arial 15 ')
            canvas.create_text(mode.width/2, mode.height*2/3, 
                            text = 'Press any key to start doodling!', 
                            font = 'Arial 15')
...
# In GameMode, doodles created by users are dropped into the world
# Users can chase after doodles and collect them into the library
# For now, it is fiding the shortest path from start to target using A* search
...
class GameMode(Mode):
    ...
    # Group 2 - Pathfinding
    #########################
    #finding the shortest path from start to targe
...
# The following part is cited from CMU 15112 course website as placeholder
class HelpMode(Mode):
    def redrawAll(mode, canvas):
        font = 'Arial 26 bold'
