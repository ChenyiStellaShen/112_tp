...
def castRay(app):
    ...
    while trial < app.rows:
        ...
        if ((0 <= rayRow <= app.rows - 1) and (0 <= rayCol <= app.cols - 1) and 
            app.map[rayRow][rayCol] == 1):
            print(f'{app.map[rayRow][rayCol]}')
            return
        """ elif rayRow < 0 and rayCol < 0:
            print("10")
            app.rX = app.margin
            app.rY = app.margin
            trial += 1
            # return
        elif rayRow > app.rows - 1 and rayCol < 0:
            print("11")
            app.rX = app.margin
            app.rY = app.height - app.margin
            trial += 1
            # return
        elif rayRow < 0 and rayCol > app.cols - 1:
            print("12")
            app.rX = app.width - app.margin
            app.rY = app.margin
            trial += 1
            # return
        elif rayRow > app.rows - 1 and rayCol > app.cols - 1:
            print("13")
            app.rX = app.width - app.margin
            app.rY = app.height - app.margin
            trial += 1
            # return
        elif rayCol > app.cols - 1:
            print("14")
            app.rX = app.width - app.margin
            trial += 1
        elif rayCol < 0:
